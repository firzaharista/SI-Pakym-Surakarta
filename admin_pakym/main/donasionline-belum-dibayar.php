...
<script language="JavaScript" type="text/javascript">
    function confirmDelete() {
        if (confirm("Donasi Yakin Dibatalkan ?")) {
            return true;
        } else {
            return false;
        }
                                ...
                                <div class="table-responsive">
                                    <table id="dataTable1" width="100%" class="table table-striped table-lightfont">
                                        ...
                                        <tbody>
                                            <?php
                                            $no = 1;
                                            $sql = "SELECT * FROM donasi_online,bank_tujuan,waktu_donasionline WHERE status_donasi = 'Donasi Belum Dibayar' AND donasi_online.id_banktujuan = bank_tujuan.id_banktujuan AND waktu_donasionline.no_invoice = donasi_online.id_donasionline group by id_donasionline order by id_donasionline DESC";
                                            $run = mysqli_query($conn, $sql);
                                            while ($c = mysqli_fetch_array($run)) {
                                                // $waktu = date("d-m-Y, H:i:s", strtotime($c['waktu_online']));
                                                $waktu = date("d-m-Y, H:i:s", strtotime($c['waktu_donasi']));
                                                echo "
                                                    <tr align='center'>
                                                        <td>$no</td>
                                                        ...
                                                        <td>$waktu WIB</td>
                                                        <td>
                                                            <a href='?page=detail-donasionline-belum-dibayar&detail=$c[id_donasionline_en]'><input type='button' class='btn btn-outline-success' value='Detail'></a>
                                                            <a href='?page=don-dibatalkan&dibatalkan=$c[id_donasionline_en]' onClick='return confirmDelete()'><input type='button' class='btn btn-outline-danger' value='Dibatalkan'></a>
                                                        </td>
                                                    </tr>";
                                                $no++;
                                            }
